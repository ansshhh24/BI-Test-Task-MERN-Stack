...
exports.userReg = async (req, res) => {
    try {
        ...
        // If email ID is present in database throws error and return message
        if (user) {
            ...
            const isEmailValid = validator.emailValidation(newUser.emailId);
            const isPasswordValid = validator.passwordValidation(newUser.password);
            const isFirstNameValid = validator.notNull(newUser.firstName);
            ...
            if (isEmailValid && isPasswordValid && isFirstNameValid) {
                ...
                var savedUser = await model.User.create(newUser);
                res.status(200).json({
                    status: "Success",
                ...
                if (!isEmailValid) validationMessage += 'Invalid Email. ';
                if (!isPasswordValid) validationMessage += 'Invalid Password. ';
                if (!isFirstNameValid) validationMessage += 'First Name cannot be null. ';
